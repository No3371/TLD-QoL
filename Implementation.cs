...
namespace QoL
{
	...
	// internal class ContainerSlotVisualPatch
	// {
	// 	private static void Postfix(Panel_Container __instance)
	// 	{
	// 		if (Settings.options.containerSlotLimitRatio < 0.1f) return;
	// 		MelonLogger.Msg($"[QoL] RefreshTables");

    //         Container container = __instance.m_Container;
    //         var slotLimit = container.m_Capacity.ToQuantity(1f) * Settings.options.containerSlotLimitRatio;

	// 		for (int i = 0; i < __instance.m_ContainerUIComponent.m_ContainerTableItems.Count; i++)
	// 		{
	// 			var itx = __instance.m_ContainerFirstItemDisplayedIndex + i;
	// 		}
	// 	}
	// }
